...
- Purpose: anchor the project field that the MCP smart-parse loop writes into `workLogs`. The vocabulary builds up incrementally during the smart-parse loop — when a debrief bullet mentions a project not yet in `known`, the agent asks the user to confirm and calls `add` (UPSERT). There is no upfront enumeration ritual; the user should not be asked to list their projects before any debriefs have been fetched. `list` is for review (e.g. before Netdok mapping, or when the user asks "what projects do I have"); `set` is for explicit cleanup or full replacement. Subsequent sessions reload the persisted list automatically.
